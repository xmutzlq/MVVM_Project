package google.architecture.coremodel.datamodel.http.progress;
...
import google.architecture.coremodel.datamodel.http.progress.body.ProgressRequestBody;
import google.architecture.coremodel.datamodel.http.progress.body.ProgressResponseBody;
...
public final class ProgressManager {
    ...
    private static final String IDENTIFICATION_NUMBER = "?param=";
    private static final String IDENTIFICATION_HEADER = "param";
    ...
    public void setKeyReadyListener(KeyReadyListener keyReadyListener) {
        this.keyReadyListener = keyReadyListener;
    }
     ...
     * @return
     */
    public Request wrapRequestBody(Request request) throws IOException {
        if (request == null)
            return request;

        String key = request.url().toString();
        if(keyReadyListener != null) keyReadyListener.onKeyReady(key);
        LogUtils.tag("zlq").e("key_request = " + key);
        request = pruneIdentification(key, request);

        if (request.body() == null)
            return request;
        if (!TextUtils.isEmpty(key) && mRequestListeners.containsKey(key)) {
            List<ProgressListener> listeners = mRequestListeners.get(key);
            LogUtils.tag("zlq").e("contentLength = " + request.body().contentLength());
    ...
    public Response wrapResponseBody(Response response) {
        ...
        LogUtils.tag("zlq").e("key_response = " + key);
